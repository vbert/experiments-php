...
$sql = vsprintf("SELECT id,startdate,enddate,meta FROM `holiday_periods` 
WHERE YEAR(`startdate`) = YEAR('%s') AND MONTH(`startdate`) = MONTH('%s') 
    OR YEAR(`enddate`) = YEAR('%s') AND MONTH(`enddate`) = MONTH('%s') 
ORDER BY `startdate`", [
    $requYMD,
    $requYMD,
    $requYMD,
...
$sqlXX = vsprintf("SELECT id,startdate,enddate,meta FROM `holiday_periods` 
WHERE YEAR(`startdate`) = YEAR('%s') AND MONTH(`startdate`) = MONTH('%s')
    AND YEAR(`enddate`) = YEAR('%s') AND MONTH(`enddate`) = MONTH('%s')
ORDER BY `startdate`", [
    $requYMD,
    date('Y-m-d', strtotime($requYMD .' - 1 month')),
    $requYMD,
    date('Y-m-d', strtotime($requYMD .' + 1 month'))
]);
$resultXX = mysqli_query($db, $sqlXX);
...
<body class="holidaysm">
    <div id="nav">
        ...
        <div id="flags">
            ...
            <div title="Polska" id="setPL" class="poland"></div> 
        </div>
        <div id="calholdr">
    ...
    <div id="main">
        <div class="tabcont t_pl">
            <h1>Wakacje w Polsce (<?php echo $mnthyear; ?>)</h1>
            <?php
                // output holiday table for Poland
                echo getAllHolidays('pl');
            ?>
        </div>
